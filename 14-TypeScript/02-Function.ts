...

{
  ...
  const add = (a: number = 100, b: number = 50) => a + b;
  ...
  const printL: (x: number | string) => void = (x) => console.log(x);
  printL('Sunny'); // Sunny
...

{
  ...
  const person2 = { ...person }; // copied by value
  person.name = 'Alok';
...
{
  // rest parameters
  const addAll = (a: number, ...nums: number[]) => {
  ...
  const maxIn3 = (...nums: [number, number, number]) => {
